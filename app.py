...
# --- דף עבודה (DASHBOARD) ---
    ...
    if st.button("GENERATE MAGIC ✨"):
        if topic:
            with st.spinner("סוכנות AI מעבדת נתונים ומוצאת תמונה..."):
                ...
                # 2. תרגום שקט לאנגלית כדי למצוא תמונה מדויקת
                trans = client.chat.completions.create(
                    model="llama-3.1-8b-instant",
                    messages=[{"role":"system","content":"Translate the topic to 2 English keywords for luxury product photography. Only keywords."},
                              {"role":"user","content":topic}]
                )
                en_kw = trans.choices[0].message.content.strip().replace(" ", ",")
                
                # 3. משיכת תמונה משרת יציב (Unsplash)
                # שימוש בפורמט שתמיד עובדת ומביאה תמונה רלוונטית
                st.session_state.last_image_url = f"https://source.unsplash.com/featured/1024x1024?luxury,fashion,{en_kw}"
