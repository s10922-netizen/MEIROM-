...
# --- עיצוב ZARA LUXE (משודרג עם צבע וסטייל) ---
st.markdown("""
<style>
    ...
    /* כותרת יוקרתית עם גרדיאנט זהב */
    .brand-title {
        font-size: 55px;
        font-weight: 700;
        margin-top: 40px;
        letter-spacing: 5px;
        background: linear-gradient(45deg, #000, #d4af37, #000);
    ...
    /* כפתורי ZARA שחורים שהופכים לזהב */
    .stButton>button {
...
# --- דף כניסה והרשמה (הפיצ'רים שלך נשמרים כאן) ---
    ...
    with t2:
        ...
        if st.button("CREATE ACCOUNT"):
            if reg_email and reg_pass:
                ...
                st.success("נרשמת בהצלחה! עברי לטאב כניסה.")
...
# --- דף עבודה (Dashboard) ---
    ...
    topic = st.text_area("מה ה-AI ייצר עבור העסק היום?", placeholder="למשל: בושם יוקרתי, קולקציית ערב...")
    
    if st.button("GENERATE MAGIC ✨"):
        if topic:
            with st.spinner("יוצר קסם..."):
                # 1. טקסט מ-Groq
                res = client.chat.completions.create(
                    model="llama-3.3-70b-versatile",
                    messages=[{"role":"system","content":"אתה סוכן תוכן יוקרתי בסטייל ZARA. כתוב פוסט מרגש וקצר."}, {"role":"user","content":topic}]
                )
                st.session_state.last_text = res.choices[0].message.content
                
                # 2. קישור תמונה משופר (Pollinations)
                safe_topic = urllib.parse.quote(topic)
                # הוספתי seed אקראי כדי למנוע טעינה של תמונה שבורה מהזיכרון
                st.session_state.last_image_url = f"https://image.pollinations.ai/prompt/luxury,high-end,minimalist,editorial,fashion,{safe_topic}?width=1024&height=1024&nologo=true&seed={int(time.time())}"
                st.session_state.magic_done = True
        else:
            st.warning("אנא כתבי נושא")

    if st.session_state.magic_done:
        # הצגת התמונה בתוך מסגרת מעוצבת
        st.markdown(f'<div class="image-container"><img src="{st.session_state.last_image_url}" style="width:100%;"></div>', unsafe_allow_html=True)
